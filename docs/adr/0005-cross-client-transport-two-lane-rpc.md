...
[architecture#61](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0061-one-client-transport.md), which retired GraphQL entirely; the
rest of this record stands.
**Date:** 2026-07-21

## Context

[architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md) chose one persistent, bidirectional
...
exists to make this possible ([contracts#1](0001-server-driven-ui-and-the-shell.md),
[contracts#3](0003-sdui-contract-repository.md)). The transport underneath it must
...
- **One-socket-carries-everything is the exception, not the norm.** The industry
  ...
  [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md)'s single bidirectional socket carrying navigation, input, actions and
...
- **SSE was the wrong second guess.** [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md) considered "HTTP intents + SSE for
...
- **The contract is generated, and native codegen prefers protobuf.** The SDUI
  ...
  [contracts#3](0003-sdui-contract-repository.md)). Extending that to Swift, Kotlin
...
travels as one server-streaming call. This supersedes [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md)'s bespoke
WebSocket as the client transport and folds [architecture#33](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0033-supervisor-driven-live-handover.md)'s handover into stream
resume.**
...
- **Lane 1 — intents (unary).** `Navigate`, `Invoke`, `SubmitInput` (a
  ...
  ([architecture#16](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0016-published-contract-surface.md)) and the caller model
  ([architecture#17](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0017-how-a-capability-acts.md)) are unchanged — this is a
...
- **The region-update protocol is contract-owned and transport-agnostic.** The
  ...
  ([architecture#29](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0029-sdui-emit-side.md), [architecture#31](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0031-server-owned-app-shell.md))
...
- **Search-as-you-type still coalesces server-side.** `SubmitInput` calls land
  ...
  The backstop of [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md) is kept; only its mechanism moves (see Consequences).
...
- **(b) Generate `UINode` as protobuf too.** Full binary efficiency and native
  ...
  repository ([contracts#3](0003-sdui-contract-repository.md)) and its generation
  pipeline.
...
**Keep the [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md) WebSocket, upgrade the payload to protobuf.** *Considered,
...
1. **The concurrency surface grows, and needs the right pattern.** [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md)'s
...
3. **HTTP/2 is strongly preferred.** The one-connection multiplexing of the two
   ...
   ([architecture#4](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0004-supervisor-as-host-manager.md)) should account for this.
4. **Stream resume replaces the [architecture#33](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0033-supervisor-driven-live-handover.md) handover.** The `Subscribe` stream
   carries a monotonic sequence; on reconnect the client presents the last
   sequence it saw and the server replays or rebuilds. This subsumes [architecture#33](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0033-supervisor-driven-live-handover.md)'s
...
5. **A second contract framing enters the codebase.** protobuf/Connect sits
   ...
   consistent with the schema-first ethos ([architecture#16](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0016-published-contract-surface.md))
   but is another generator, another set of generated bindings, and a versioning
   surface of its own.
6. **This supersedes an Accepted ADR.** [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md)'s
   WebSocket transport is replaced and [architecture#33](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0033-supervisor-driven-live-handover.md)
   is revised on acceptance. The *goals* of [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md) — streaming input, server
   push, region updates, a technology-agnostic client — are unchanged and fully
   met; only the wire changes.

### The GraphQL question

> **Superseded by [architecture#61](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0061-one-client-transport.md).** The recommendation
...
> ended up implemented twice. [architecture#61](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0061-one-client-transport.md) takes the *consolidate* option this
...
The Platform serves a GraphQL HTTP API today, and [architecture#32](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0032-live-session-websocket.md)'s socket reuses its
...
service SessionService {
  ...
  // command/query the HTTP path would (the command boundary, [architecture#16](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0016-published-contract-surface.md)).
  ...
  // (superseding [architecture#33](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0033-supervisor-driven-live-handover.md)'s handover).
  rpc Subscribe(SubscribeRequest) returns (stream ServerMessage);
}

message SubscribeRequest {
  string session = 1;         // opaque session ref ([architecture#17](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0017-how-a-capability-acts.md)); auth is on this call
...
message RegionUpdate {
  string region = 1;          // a named slot ([architecture#29](https://github.com/mosaic-media/architecture/blob/main/docs/adr/0029-sdui-emit-side.md) / 0031)
  Op     op     = 2;
