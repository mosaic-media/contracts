...
// primitive is one entry of the native tier. Native is the justification for why
// this cannot be a definition, and lint rejects a primitive without one: growing
// the tier costs a client release per addition, and an unjustified addition
// spends that silently.
...
func main() {
	...
	// Every file this tool generates. A new entry must also be added to
	// scripts/check-generated.sh's GENERATED list, which is what diffs the
	// committed copies: one missing there is an output the drift guard
	// regenerates and then never compares.
	outputs := []struct {
		path string
		want []byte
	}{
		...
		{filepath.Join(root, "REFERENCE.md"), genReference(sp)},
	}
...
// the action/tone/surface enum values, and the primitive tier as data, so a
// consumer can ask what the contract contains rather than reading a client's
// source.
...
// would be a fourth place the vocabulary is written down.
//
// The audience is someone implementing a client, so it carries the things only
// this file knows: why each primitive is native, what a client must implement
// versus what it is served, and which sets are closed.
func genReference(sp spec) []byte {
	var b strings.Builder
	w := func(format string, a ...any) { fmt.Fprintf(&b, format, a...) }

	w("<!-- Generated by tools/genui from ui.spec.json. DO NOT EDIT. -->\n\n")
	w("# The Mosaic SDUI vocabulary\n\n")
	w("**Version %s.** This is the whole of what a client must implement and what a\n", sp.Version)
	w("producer may emit. It is generated from `ui.spec.json`, so it cannot disagree\n")
	w("with the bindings, the conformance fixture or the corpus — all four come from\n")
	w("that one file.\n\n")

	w("Three tiers, and the difference between them is what a change costs:\n\n")
	w("| Tier | What it is | Cost of adding one |\n|---|---|---|\n")
	w("| **Primitives** (%d) | Native code every client implements | A client release, on every platform |\n", len(sp.Primitives))
	w("| **Components** (%d) | Definitions the Platform serves as data | Nothing — a file in `definitions/` |\n", countComponents(sp))
	w("| **Actions** (%d) | Behaviours a client interprets | A client release |\n\n", len(sp.Actions))

	w("## Type names\n\n")
	w("A core type is unprefixed. A module's own type is `moduleId%stype`, and a core\n", sp.TypeSeparator)
	w("type never contains `%s`. Without the namespace two modules could both\n", sp.TypeSeparator)
	w("contribute a `StatChip`, and one could contribute a `PosterCard` and take the\n")
	w("core component's place in every client's registry.\n\n")

	w("## Prop values\n\n")
	w("A prop is a literal, or a binding: `{\"%s\": \"path\"}`, resolved where the node\n", sp.BindingMarker)
	w("renders. A binding names a path and nothing else — no formatting, no default,\n")
	w("no expression. Formatting is server-side, which is why no client needs an\n")
	w("evaluator. An object carrying `%s` beside any other key is a literal object.\n\n", sp.BindingMarker)

	w("## Primitives\n\n")
	w("Every one carries the reason a definition cannot express it. That justification\n")
	w("is required, because growing this tier is the only vocabulary change that costs\n")
	w("a release on every platform at once.\n\n")
	for _, p := range sp.Primitives {
		w("### `%s`\n\n", p.Type)
		w("%s\n\n", p.Doc)
		w("*Native because:* %s\n\n", p.Native)
		w("*Tier:* %s · *Renders children:* %t\n\n", p.Tier, p.Children)
		if len(p.Props) > 0 {
			w("| Prop | Type | Meaning |\n|---|---|---|\n")
			for _, pr := range p.Props {
				w("| `%s` | `%s` | %s |\n", pr.Key, pr.Type, pr.Doc)
			}
			w("\n")
		}
	}
	...
	w("## Components\n\n")
	w("Served as data (`definitions/*.json`). A client implements none of them; it\n")
	w("expands whatever it is sent.\n\n")
	for _, c := range sp.Components {
		if c.Generic || c.Type == "" {
			continue
		}
	...
	w("## Closed sets\n\n")
	w("Each of these is closed, and closed in the same direction: the server may name\n")
	w("only what every client can interpret. An open set lets the server state\n")
	w("something a client silently ignores, which fails open.\n\n")

	w("**Validators** — a rule the server states and the client enforces. The server\n")
	w("enforces everything again; the client's copy is what saves the round trip.\n\n")
	w("| Validator | Argument | Meaning |\n|---|---|---|\n")
	for _, v := range sp.Validators {
		w("| `%s` | `%s` | %s |\n", v.Name, v.Arg, v.Doc)
	}
	w("\n")

	w("**Predicates** — conditions over scope values, as `visibleWhen` uses. An\n")
	w("unreadable predicate is **false**: a control shown because its rule could not\n")
	w("be read is the failure that reveals things.\n\n")
	w("| Predicate | Shape |\n|---|---|\n")
	for _, p := range sp.Predicates {
		w("| `%s` | %s |\n", p.Name, p.Doc)
	}
	...
	w("**Roles** — mapped, not passed through: ARIA on the web, a semantics property\n")
	w("in Compose, a trait in SwiftUI. A role a client maps to nothing is a control\n")
	w("invisible to a screen reader and correct-looking to everyone else.\n\n")
	w("%s\n\n", codeList(sp.Roles))

	w("**Focus directions** — what a `nextFocus` override may name. No diagonals: no\n")
	w("remote has them.\n\n")
	w("%s\n\n", codeList(sp.FocusDirs))

	w("**Tones**\n\n%s\n\n", codeList(toneValues(sp)))
	w("**Overlay surfaces**\n\n%s\n\n", codeList(surfaceValues(sp)))

	w("## Proving a client implements this\n\n")
	w("`conformance/vocabulary.json` is this document as data — the sets a client\n")
	w("declares itself against. `conformance/cases/*.json` is the corpus: golden\n")
	w("inputs and outputs for validators, predicates, binding resolution and\n")
	w("definition expansion. A client that passes the corpus behaves like the ones\n")
	w("that exist; a client that does not knows exactly where it differs.\n")

	return []byte(b.String())
}

func countComponents(sp spec) int {
	n := 0
	for _, c := range sp.Components {
		if !c.Generic && c.Type != "" {
			n++
...
func toneValues(sp spec) []string {
	out := make([]string, 0, len(sp.Tones))
	for _, t := range sp.Tones {
...
func surfaceValues(sp spec) []string {
	out := make([]string, 0, len(sp.Surfaces))
	for _, s := range sp.Surfaces {
...
func runLint(sp spec, root string) []string {
	...
	// Action-kind, tone and surface coverage: the spec, the JSON Schema and the
	// proto are the three places the vocabulary is written down, and they must
	// agree.
	...
	for _, f := range files {
		...
		for b := range binds {
			...
			// variable, not a component prop. Match on the path's head rather than
			// the bare alias: a repeated node binds dotted paths, and matching only
			// the bare alias fails every one of them.
...
//
// Nothing else compares the three, so without this a kind can exist in one and
// not the others with every build still green: the sets simply disagree.
